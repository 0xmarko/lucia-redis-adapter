// Adapted from @lucia-auth/adapter-test
// Tests the session-only Redis adapter, which doesn't store user data.
...
export async function testAdapter(adapter: Adapter) {
  ...
  await test('updateSessionExpiration() updates session', async () => {
    ...
    databaseSession.expiresAt = new Date(
      databaseSession.expiresAt.getTime() + 10_000,
    );
    await adapter.updateSessionExpiration(
      databaseSession.id,
